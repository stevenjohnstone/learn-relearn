...
def brownian_cov(samples):
    return [[(1/samples)*min(i,j) for i in range(1, samples)] for j in range(1, samples)]

    
def brownian_path(samples, A):
    return np.r_[0, A.dot(rg.normal(0, 1, samples-1))]
    

samples = 100
A_brownian = np.linalg.cholesky(brownian_cov(samples))
x_axis = np.linspace(0, 1, samples)

for _ in range(100):
    plt.plot(x_axis, brownian_path(samples, A_brownian))
```
...
```{python}
def ou_cov(samples):
    return [ [ (1/2)*np.exp(-(2/samples) * np.abs(i -j))*(1 - np.exp(-(2/samples)*min(i, j))) for i in range(1, samples)] for j in range(1, samples)]

def ou_path(samples, A):
    return np.r_[0, A.dot(rg.normal(0, 1, samples-1))]
    
A_ou = np.linalg.cholesky(ou_cov(samples))

for _ in range(100):
    plt.plot(x_axis, ou_path(samples, A_ou))
```
```{python}
def ou_cov_stationary(samples):
    return [ [ (1/2)*np.exp(-(2/samples) * np.abs((i -j))) for i in range(samples)] for j in range(samples)]

def ou_stationary_path(samples, A):
    return A.dot(rg.normal(0, 1, samples))
    
A_ou_stationary = np.linalg.cholesky(ou_cov_stationary(samples))

for _ in range(100):
    plt.plot(x_axis, ou_stationary_path(samples, A_ou_stationary))
```
...
def frac_brownian_cov(samples, H):
    def cov(i,j):
        scale = 1/samples
        i, j = scale*i, scale*j
        return (1/2) *(i**(2*H) + j**(2*H) - np.abs(i -j)**(2*H))
    return [[cov(i, j) for i in range(1, samples)] for j in range(1, samples)]
    
    
def frac_brownian_path(samples, A):
    return np.r_[0, A.dot(rg.normal(0, 1, samples-1))]
    

for H in [0.1, 0.5, 0.9]:
    A = np.linalg.cholesky(frac_brownian_cov(samples, H))
    for _ in range(100):
        plt.plot(x_axis, frac_brownian_path(samples, A))
    plt.title(f'H = {H}')
    plt.show()
```
...
```{python}
def bridge_cov(samples):
    def cov(i, j):
        scale = 1/samples
        i, j = scale*i, scale*j
        return min(i,j) - i*j

    return [[cov(i,j) for i in range(1, samples -1)] for j in range(1, samples -1)]

def bridge_path(samples, A):
    return np.r_[np.r_[0, A.dot(rg.normal(0, 1, samples-2))], 0]
    
A_bridge = np.linalg.cholesky(bridge_cov(samples))
    
for _ in range(100):
    plt.plot(x_axis, bridge_path(samples, A_bridge))
...
```{python}
def bridge_from_brownian(samples, A):
    path = brownian_path(samples, A)
    return [ path[i] - (i/samples) * path[-1] for i in range(len(path)) ]

for _ in range(100):
    plt.plot(x_axis, bridge_from_brownian(samples, A_brownian))
```
...
path_a = bridge_path(samples, A_bridge)
rg = np.random.default_rng(1337)
path_b = bridge_from_brownian(samples, A_brownian)
